...
void BFSGraph(int argc, char** argv) {
  ...
  buffer<bool> h_updating_graph_mask_buf(h_updating_graph_mask,
                                         range<1>(no_of_nodes));
  ...
  q.submit([&](handler& cgh) {
    auto h_graph_mask_buf_access =
        h_graph_mask_buf.get_access<access::mode::read_write>(cgh);
    auto h_graph_visited_buf_access =
        h_graph_visited_buf.get_access<access::mode::read_write>(cgh);
    auto h_updating_graph_mask_buf_access =
        h_updating_graph_mask_buf.get_access<access::mode::read_write>(cgh);
    auto h_graph_nodes_buf_access =
        h_graph_nodes_buf.get_access<access::mode::read>(cgh);

    auto h_graph_edges_buf_access =
        h_graph_edges_buf.get_access<access::mode::read>(cgh);
    auto h_cost_buf_access =
        h_cost_buf.get_access<access::mode::read_write>(cgh);
        ...
        for (int tid = 0; tid < no_of_nodes_access[0]; tid++) {
          if (h_graph_mask_buf_access[tid] == true) {
            ...
            for (int i = h_graph_nodes_buf_access[tid].starting;
                 i < (h_graph_nodes_buf_access[tid].no_of_edges +
                      h_graph_nodes_buf_access[tid].starting);
                 i++)
            {
