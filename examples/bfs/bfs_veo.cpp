...
void BFSGraph(int argc, char **argv) {
  ...
  rc = veo_alloc_mem(proc, &ve_h_updating_graph_mask, sizeof(bool) * no_of_nodes);
  if (rc != VEO_COMMAND_OK) {
    perror("allocate memory return bad code\n");
  }
  ...
  // do data copy
  rc = veo_write_mem(proc, ve_h_graph_nodes, h_graph_nodes, sizeof(Node) * no_of_nodes);
  if (rc != VEO_COMMAND_OK) {
    perror("write memory return bad code\n");
  }

  rc = veo_write_mem(proc, ve_h_graph_mask, h_graph_mask, sizeof(bool) * no_of_nodes);
  if (rc != VEO_COMMAND_OK) {
    perror("write memory return bad code\n");
  }

  rc = veo_write_mem(proc, ve_h_updating_graph_mask, h_updating_graph_mask, sizeof(bool) * no_of_nodes);
  if (rc != VEO_COMMAND_OK) {
    perror("write memory return bad code\n");
  }

  rc = veo_write_mem(proc, ve_h_graph_visited, h_graph_visited, sizeof(bool) * no_of_nodes);
  if (rc != VEO_COMMAND_OK) {
    perror("write memory return bad code\n");
  }

  rc = veo_write_mem(proc, ve_h_graph_edges, h_graph_edges, sizeof(int) * edge_list_size);
  if (rc != VEO_COMMAND_OK) {
    perror("write memory return bad code\n");
  }
  ...
  //copy data back
  rc = veo_read_mem(proc, h_graph_mask, ve_h_graph_mask, sizeof(bool) * no_of_nodes);
  if (rc != VEO_COMMAND_OK) {
    perror("read memory return bad code\n");
  }

  rc = veo_read_mem(proc, h_graph_visited, ve_h_graph_visited, sizeof(bool) * no_of_nodes);
  if (rc != VEO_COMMAND_OK) {
    perror("read memory return bad code\n");
  }

  rc = veo_read_mem(proc, h_updating_graph_mask, ve_h_updating_graph_mask, sizeof(bool) * no_of_nodes);
  if (rc != VEO_COMMAND_OK) {
    perror("read memory return bad code\n");
  }
