  ...
  for (Decl *d : lambda_func_decl->getParent()->decls()) {
    if (isa<VarDecl>(d)) {
      ...
      ClassTemplateSpecializationDecl *template_decl =
          clang_cast<ClassTemplateSpecializationDecl>(raw_decl);
          ...
          if (type_decl->getIdentifier()) {
            ...
            if (context.structs.count(def_name) == 0) {
              context.structs.insert(
                  std::pair<std::string, std::string>(def_name, def_body));
            }

#ifdef DEBUG
            std::cout << "Definition name: " << def_name << std::endl;
            std::cout << "========= Definition body start ========="
                      << std::endl;
            std::cout << def_body << std::endl;
            std::cout << "=========  Definition body end  ========="
                      << std::endl;
#endif
          }
        }
      ...
      PRINT_INFO(
          "Found a lambda field decl, Type: {}, Name: {}, Dimensions: {}",
          field_type, name, field_dimensions);
    }
  }
