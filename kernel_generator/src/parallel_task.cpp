...
KernelInfo parse_parallel_task_func(CompilerInstance &ci, const FunctionDecl *callee, ProgramContext &context) {
  ...
  const TemplateArgumentList *template_args = callee->getTemplateSpecializationArgs();
  ...
  clang::MangleContext *mangleContext = ci.getASTContext().createMangleContext();
  ...
  std::string classname = classname_type->getAsRecordDecl()->getDeclName().getAsString();

  CXXRecordDecl *lambda_func_decl = lambda_func_type->getAsCXXRecordDecl();

  std::vector<KernelArgument> kernel_arguments = analyze_arguments_dependency(ci, lambda_func_decl, context);
  ...
  std::string index_name = lambda_decl->getParamDecl(0)->getIdentifier()->getName().str();

#ifdef DEBUG
  std::cout << "Parallel kernel name: " << kernelName << " , index_name: " << index_name << std::endl;
