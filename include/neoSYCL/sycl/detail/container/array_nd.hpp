...
#define DEFINE_ARRAY_ND_OP_CONST(op)                                                                        \
  friend ArrayND<dimensions> operator op(const ArrayND<dimensions> &lhs, const ArrayND<dimensions> &rhs) {  \
...
#define DEFINE_ARRAY_ND_OP_CONST_SIZE_T(op)                                                                 \
  friend ArrayND<dimensions> operator op(const ArrayND<dimensions> &lhs, const size_t &rhs) {               \
    ArrayND<dimensions> ret;                                                                                \
    for(std::size_t i = 0; i < dimensions; ++i) {                                                           \
...
#define DEFINE_ARRAY_ND_OP(op)                                                                              \
  friend ArrayND<dimensions> &operator op(ArrayND<dimensions> &lhs, const ArrayND<dimensions> &rhs) {       \
    for(std::size_t i = 0; i < dimensions; ++i) {                                                           \
...
#define DEFINE_ARRAY_ND_OP_SIZE_T(op)                                                                       \
  friend ArrayND<dimensions> &operator op(ArrayND<dimensions> &lhs, const size_t &rhs) {                    \
    for(std::size_t i = 0; i < dimensions; ++i) {                                                           \
...
#define DEFINE_ARRAY_ND_OP_CONST_SIZE_T_LEFT(op)                                                            \
  friend ArrayND<dimensions> operator op(const size_t &lhs, ArrayND<dimensions> &rhs) {                     \
    ArrayND<dimensions> ret;                                                                                \
    for(std::size_t i = 0; i < dimensions; ++i) {                                                           \
...
friend bool operator==(const ArrayND<dimensions> &lhs, const ArrayND<dimensions> &rhs) {                    \
    for(std::size_t i = 0; i < dimensions; ++i) {                                                           \
      if(lhs[i]!=rhs[i]){                                                                                   \
        return false;                                                                                       \
      }                                                                                                     \
