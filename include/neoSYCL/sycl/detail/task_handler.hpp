...
public:
  ...
  virtual void run(shared_ptr_class<kernel> k) = 0;
  ...
  virtual void set_capture(shared_ptr_class<kernel>, void* p, size_t sz) = 0;
  virtual void set_range(shared_ptr_class<kernel>, size_t r[6])          = 0;
...
public:
  ...
  virtual void run(shared_ptr_class<kernel> k) {
  ...
  void* alloc_mem(container_type p,
                  access::mode = access::mode::read) override {
    return nullptr;
  ...
  void set_capture(shared_ptr_class<kernel> k, void* p, size_t sz) {
    auto ki = std::dynamic_pointer_cast<detail::kernel_info_cpu>(
        k->get_kernel_info());
    if (ki && ki->capt_)
