...
friend bool operator op(const cls<dimensions> &lhs, const cls<dimensions> &rhs) {                    \
    return lhs.data op rhs.data;                                                                     \
}

#define DEFINE_COMMON_BY_VALUE_SEMANTICS(cls) DEFINE_COMMON_BY_VALUE_OP(cls,==); DEFINE_COMMON_BY_VALUE_OP(cls,!=)
