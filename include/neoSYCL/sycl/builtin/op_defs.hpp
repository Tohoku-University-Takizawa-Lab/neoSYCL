...
#define DEFINE_GEN_FUNC(N, F)                                                  \
  template <typename T> T N(const T& i) { return F(i); }                       \
  template <typename T, size_t D> vec<T, D> N(const vec<T, D>& x) {            \
    vec<T, D> R;                                                               \
...
#define DEFINE_UGEN_FUNC(N, F)                                                 \
  template <typename T> typename std::make_unsigned<T>::type N(const T& i) {   \
    return F(i);                                                               \
  }                                                                            \
...
#define DEFINE_GEN_FUNC2(N, F)                                                 \
  template <typename T> T N(const T& x, const T& y) { return F(x, y); }        \
  template <typename T, size_t D>                                              \
...
#define DEFINE_GEN_FUNC3(N, F)                                                 \
  template <typename T> T N(const T& x, const T& y, const T& z) {              \
    return F(x, y, z);                                                         \
  }                                                                            \
  template <typename T, size_t D>                                              \
...
#define DEF_VEC_INCDEC_OPERATOR(OP)                                            \
  template <typename T, size_t D> vec<T, D>& operator OP(vec<T, D>& lhs) {     \
    for (size_t i(0); i < D; i++) {                                            \
      OP(lhs.data_[i]);                                                        \
    }                                                                          \
    return lhs;                                                                \
...
#define DEF_VEC_UNARY_OPERATOR(OP)                                             \
  template <typename T, size_t D> vec<T, D> operator OP(const vec<T, D>& v) {  \
    vec<T, D> r;                                                               \
    for (size_t i(0); i < D; i++) {                                            \
