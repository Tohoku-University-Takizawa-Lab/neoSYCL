...
struct VEKernel : public Kernel {
  ...
  VEKernel(const vector_class<KernelArg> &args, const string_class &kernel_name,
           const nec::VEProc &proc)
    ...
    for (int i = 0; i < args.size(); i++) {
      ...
      if (arg.mode != access::mode::write) {
        ...
        rt = veo_write_mem(proc.ve_proc, ve_addr_int,
                           arg.container->get_data_ptr(), size_in_byte);
        if (rt != veo_command_state::VEO_COMMAND_OK) {
          DEBUG_INFO(
    ...
    for (int i = 0; i < args.size(); i++) {
      ...
      if (arg.mode != access::mode::read) {
        DEBUG_INFO("[VEKernel] copy from ve memory, device address: {:#x}, "
                   ...
                   (size_t)arg.container->get_data_ptr());
        // do copy
        int rt = veo_read_mem(proc.ve_proc, arg.container->get_data_ptr(),
  ...
  void parallel_for(const range<1> &r) override {
    DEBUG_INFO("[VEKernel] parallel for 1d {} with range: {}", kernel_name,
               r.size());
